 ...
 */
function doGet(e) {
  // If requested as API endpoint (e.g. from Blogger / external web app)
  if (e && e.parameter && (e.parameter.action || e.parameter.api === '1' || e.parameter.callback)) {
    return handleApiRequest(e);
  }

  // Default: Serve HTML interface (standalone GAS Web App or iFrame inside Blogger)
  initDatabase();
...
/**
 * Universal API Action Dispatcher (Supports both internal GAS, REST fetch, and JSONP from Blogger)
 */
function executeApiAction(action, params) {
  ...
  if (action === 'getInitialData') {
    ...
    result = deletePost(params.postId, params.username);
  } else if (action === 'addComment') {
    result = addComment(params.comment || {});
  } else if (action === 'addReel') {
    result = addReel(params.reel || {});
  } else if (action === 'reactToReel') {
    result = reactToReel(params.reelId, params.username, params.emoji);
  } else if (action === 'addStory') {
    result = addStory(params.story || {});
  } else if (action === 'viewStory') {
    result = viewStory(params.storyId, params.username);
  } else if (action === 'deleteStory') {
    result = deleteStory(params.storyId);
  } else if (action === 'addMarketItem') {
    result = addMarketItem(params.item || {});
  } else if (action === 'addEvent') {
    result = addEvent(params.event || {});
  } else if (action === 'toggleEventRSVP') {
    result = toggleEventRSVP(params.eventId, params.username);
  } else if (action === 'toggleFollow') {
    result = toggleFollowUser(params.followerUsername, params.targetUsername);
  } else if (action === 'updateProfile') {
    result = updateFullProfile(params.userData || {});
    ...
    result = { success: true, message: 'Rong Social Apps Script API is live!', time: new Date().toISOString() };
  }
...
/**
 * Handle API calls via GET (JSON / JSONP) - Perfect for Blogger cross-origin calls
 */
function handleApiRequest(e) {
  try {
    var p = e.parameter || {};
    var action = p.action || 'getInitialData';
    var callback = p.callback || '';
    var payload = {};

    if (p.data) {
      try { payload = JSON.parse(p.data); } catch(ex){}
    } else {
      payload = p;
    }
    ...
    // If JSONP callback is provided (Blogger script tag integration)
    if (callback) {
...
/**
 * Web App HTTP POST Endpoint (for REST API / External Webhook / Blogger fetch)
 */
function doPost(e) {
  try {
    ...
    var result = executeApiAction(action, params);
